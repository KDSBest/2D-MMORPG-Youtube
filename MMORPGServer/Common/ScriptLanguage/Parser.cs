	...
	public class Parser
	{
		...
		private ASTStatement ParseStatement()
		{
			...
			for (; currentToken < currentTokens.Count; currentToken++)
			{
				...
				else if (currentTokens[currentToken] is OperatorToken)
				{
		...
		private ASTStatement ApplyOperatorPrecedences(List<ASTNode> nodes)
		{
			...
			int lastPrecedence = int.MaxValue;
			...
			for (int i = nodes.Count - 1; i >= 0; i--)
			{
				...
				{
					// Build Operator Tree
					ProcessOperatorStack(parameters, operators, true);
				}
				else
				{
					if (nodes[i] is ASTOperator && ((ASTOperator)nodes[i]).Left == null)
					{
						ASTOperator op = (ASTOperator)nodes[i];
						if (lastPrecedence < op.Precedence)
						{
							ProcessOperatorStack(parameters, operators, false);
						}
		...
		private void ProcessOperatorStack(Stack<ASTNode> parameters, Stack<ASTOperator> operators, bool removeParenthese)
		{
